...
// GetChildOrders calls the mock function if set, otherwise returns empty slice
func (m *MockBitFlyerClient) GetChildOrders(productCode string, childOrderState string) ([]model.BitflyerChildOrder, error) {
	if m.GetChildOrdersFunc != nil {
		return m.GetChildOrdersFunc(productCode, childOrderState)
	}
